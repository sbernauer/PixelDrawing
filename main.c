...
void printInformation(struct frontend* front, unsigned int actualFps) {
	char *fpsString = (char*)malloc(15 * sizeof(char));
	sprintf(fpsString, "FPS: %d", actualFps);

	frontend_draw_string(front, 10, 0, "gpn.mcarco.de:1234");
	// frontend_draw_string(front, 10, 15, "DECT: 1234");
	// frontend_draw_string(front, 10, 30, "#bitte mehr bits");
	frontend_draw_string(front, 10, 15, fpsString);

	double loadAverages[3];
	getloadavg(loadAverages, 3);
	char *loadString = (char*)malloc(500 * sizeof(char));
	sprintf(loadString, "Load: %3.1f %3.1f %3.1f", loadAverages[0], loadAverages[1], loadAverages[2]);
	frontend_draw_string(front, 10, 30, loadString);
}
	...
	while(!do_exit) {
		...
		llist_for_each(&fronts, cursor) {
			...
			if(frontend_can_draw_string(front)) {
				printInformation(front, actualFps);
			}
